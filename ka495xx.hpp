/**
 * @file ka495xx.hpp
 * @author Kyungwoo Min (bluelife85@naver.com)
 * @brief 
 * @version 0.1
 * @date 2023-07-26
 * 
 * @copyright Copyright (c) 2023
 * 
 */
#ifndef __DRIVER_KA495XX_H__
#define __DRIVER_KA495XX_H__

#include "ka495xx_sfr.hpp"

class Ka495xx
{
public:
   Ka495xx();
   Ka495xx(spi_xfer8 spi, hw_miso miso, hw_sen sen, hw_nrst nrst, hw_vpc vpc, hw_delay_ms delay);
   Ka495xx(spi_xfer8 spi, hw_miso miso, hw_sen sen, hw_nrst nrst, hw_vpc vpc, hw_delay_ms delay, hw_fet_off fet_off, hw_io3 io3, hw_io2 io2, hw_io1 io1);
private:
   hw_vpc vpc;
   hw_sen sen;
   hw_nrst nrst;
   hw_miso miso;
   hw_fet_off fet_off;
   hw_io3 io3;
   hw_io2 io2;
   hw_io1 io1;
   spi_xfer8 spi;
   hw_delay_ms delay;
   const unsigned char crcTable[256] = 
   {
      
   };

   unsigned char crc(unsigned char* data, unsigned long size);
   inline unsigned char swapbit(unsigned char data)
   {
      data = (data & 0xF0) >> 4 | (data & 0x0F) << 4;
      data = (data & 0xCC) >> 2 | (data & 0x33) << 2;
      data = (data & 0xAA) >> 1 | (data & 0x55) << 1;
      return data;
   }
};

#endif /* __DRIVER_KA495XX_H__ */
